...
export default function ProductCard() {
  ...
  return (
    <div className="">
      <div className=" flex flex-col items-center gap-4 mt-4 mb-4 px-4 md:px-8 lg:px-0">
        {productData[dataKey].map((item) => (
          <div
            key={item.id}
            className="flex py-4 px-4 md:px-8 gap-4 md:gap-8 items-center justify-start border-b-2 border-gray-250  lg:max-w-[60%] w-[100%] shadow-md rounded-lg hover:bg-[#f2f2f2] transform duration-300 ease-in-out "
          >
            <img src={item.image} alt={item.title} />
            <div className="titleDetailsSection    cursor-pointer">
              <h3 className="text-[#6d6d6d] hover:text-black transform ease-in-out duration-300 font-semibold text-lg md:text-2xl">
                {item.title}
              </h3>
              {/* <p>{item.details}</p> */}
            </div>
